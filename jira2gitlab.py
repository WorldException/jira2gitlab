...
for issue in jira_issues:
    if ISSUE_TRACKING and (issue['key'] in processed_jiras):
        ...
        if not project_queue:
            _key = issue['fields']['project']['key']
            project_queue[_key] = PROJECT_MAP[_key]
        ...
        #if issue['fields']['status']['statusCategory']['name'] in ("In Progress", "В работе"):
        labels.append(issue['fields']['status']['name'])

        # Add Epic name to labels
        if issue['fields'].get(JIRA_EPIC_FIELD):
        ...
        milestone_id = None
        milestone_name = ''
        if issue['fields'].get(JIRA_SPRINT_FIELD):
        ...
        for JIRA_PROJECT_KEY, GITLAB_PROJECT_ID in project_queue.items():
            ...
            newlabels.append(issue_info['key'])
            ...
            skip = False
            for gl_issue in project.issues.list():
                if gl_issue.title == issue['fields']['summary']:
                    print("skip "+gl_issue.title)
                    skip = True
                    break
            if skip:
                continue
